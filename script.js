  ...
  function hourTracker() {
    ...
    $(".time-block").each(function () {
      // Defining the variable 'blockHour' = 
          // Next 'parseInt' is designed to turn each loop into a integer. {It does this by taking each loop, converting it to a string, then parses that string and returns an integer.} - It references '$(this)' which is what is pulling from the localStorage.  - It then grabs each item in localStorage by the attribute '.attr' via the 'id' it was saved by. - Next it takes the 'id' and '.splits' the id by removing the word "hour" {which is referenced as a string}. So it leaves just the hour number within the id name. It needs to split the verbiage off otherwise the 'parseInt' would display 'NaN' information. Because 'hour' is only mentioned once, the split will be seperated into two pieces. Because JS index starts at 0 we will need call the value as [1] to get the section after the split.
